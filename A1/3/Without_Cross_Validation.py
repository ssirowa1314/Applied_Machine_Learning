...
class DecisionTree():
    def __init__(self):     #constructor
        #self.root=Node()
        self.tree={}        #dictionary to store tree
        self.data={}        #data at a node
        self.nodeVal=0      #number of nodes in tree


#This function learns the decision tree. usedAttr_1 contains a dictionary of the attributes which are used in a branch. Currnode is the node we are currently processing
    def learn(self, training_set, usedAttr_1={}, currNode=0):
        ...
        h=self.computeEntropy(training_set);    #compute the entropy of currnode
        if(h==0.0):                             #make leaf if node is pure
        ...
        if(mxAttr>-1):                                      #if there is any information gain for a split
            ...
            if len(usedAttr_new)<11:                        #if all attributes are used, then make leaf.
                ...
                self.learn(left,usedAttr_l,l)               #learn the left subtree
                self.learn(right,usedAttr_r,r)              #learn the right subree
        ...
        for k in range(0,15):
            ...
            for it in a:
                if it<=j:
                    ...
                    break
            a1=self.computeEntropy(x,j,i)*(float(count)/l)+self.computeEntropy(x,j,i,1)*(float(l-count)/l)
            a2=e-a1
            if mx<a2:
                mx=a2
                val=j    
...
#function to compute entory. p1 determines what is attribute index for which we are finding entropy. p2 determines the split value for the attribute. p3 determines wheter left subree or right subtree. x is training set
    def computeEntropy(self,x, p2=0.0, p1=-1, p3=0):
        count_1=0
        l=0;        #count of rows in x which are considered
        for row in x:           
            if p1!=-1 and p3==0 and float(row[p1])<=p2:         #left subtree 
                l+=1;
                if row[11] == "1":
                    count_1+=1
            elif p1!=-1 and p3==1 and float(row[p1])>p2:        #right subtree
                l+=1
                if row[11] == "1":
                    count_1+=1
            elif p1==-1:                                        #root
        ...
        pno=float(count_0)/l
        return (pyes*m.log(pyes,2)+pno*m.log(pno,2))*(-1)


    def retNodeval(self):               #function to return number of nodes
        return self.nodeVal
